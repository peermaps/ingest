...
async fn ingest() -> Result<(),Error> {
  ...
  let mut o5c_file0 = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
  o5c_file0.push("tests/data/changeset0.o5c");
  let mut o5c_file1 = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
  o5c_file1.push("tests/data/changeset1.o5c");
  let mut o5c_file2 = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
  o5c_file2.push("tests/data/changeset2.o5c");
  ...
  ingest.changeset(Box::new(File::open(&o5c_file0).await?)).await?;
  {
    let mut estore = ingest.estore.lock().await;
    let mut stream = estore.db.query(&((3.0,-15.0),(15.0,45.0))).await?;
    let mut results = vec![];
  ...
  ingest.changeset(Box::new(File::open(&o5c_file1).await?)).await?;
  {
    let mut estore = ingest.estore.lock().await;
    let mut stream = estore.db.query(&((3.0,-15.0),(15.0,45.0))).await?;
    let mut results = vec![];
  ...
  ingest.changeset(Box::new(File::open(&o5c_file2).await?)).await?;
  {
    let mut estore = ingest.estore.lock().await;
    let mut stream = estore.db.query(&((3.0,-15.0),(15.0,45.0))).await?;
    let mut results = vec![];
