  ...
  while let Some((q_bbox,q_inserts)) = queue.pop_front() {
    ...
    for iy in 0..ny {
      for ix in 0..nx {
        let bbox = (
          (
            (ix as f32 / nx as f32) * ((q_bbox.1).0 - (q_bbox.0).0) + (q_bbox.0).0,
            (iy as f32 / ny as f32) * ((q_bbox.1).1 - (q_bbox.0).1) + (q_bbox.0).1,
          ),
          (
            ((ix as f32 + 1.0) / nx as f32) * ((q_bbox.1).0 - (q_bbox.0).0) + (q_bbox.0).0,
            ((iy as f32 + 1.0) / ny as f32) * ((q_bbox.1).1 - (q_bbox.0).1) + (q_bbox.0).1,
          ),
        );
        boxes.push((bbox,vec![]));
      }
